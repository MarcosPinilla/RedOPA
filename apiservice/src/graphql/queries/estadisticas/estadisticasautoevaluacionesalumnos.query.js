...
const estadisticasAutoevaluacionesAlumnosQuery = {
  ...
  async resolve (root, { institucion_id, periodo }, { me }) {
    ...
    for (let i = 0; i < cursos.length; i++) {
      ...
      if (cursos[i].alumnos.length > 0) {
        let porcentaje = (autoevaluadosCurso / cursos[i].alumnos.length) * 100;
...
export default estadisticasAutoevaluacionesAlumnosQuery;