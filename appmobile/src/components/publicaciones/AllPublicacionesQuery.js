...
import { View, StyleSheet, SafeAreaView, ScrollView, TouchableOpacity, FlatList, RefreshControl } from 'react-native';
...
const AllPublicationsQuery = graphql(
  gql`
    query Publicaciones($offset: Int, $limit: Int) {
...
function Publications({ data }) {
  ...
  const dataItemsL = data.publicaciones?.items?.length ?? 0;
  const totalItems = data.publicaciones?.totalItems ?? 0;
  ...
  return (
    ...
    ? (data.publicaciones.items.length > 0)
      ? <View>
        <FlatList
          data={data.publicaciones.items}
          keyExtractor={item => item.id}
          refreshing={data.networkStatus === 4}
          onRefresh={() => data.refetch()}
          onEndReachedThreshold={0.01}
          onEndReached={
            (data.publicaciones.items.length < data.publicaciones.totalItems)
            ? () => loadMore(data)
            : null
          }
...
const loadMore = (data) => {
  data.fetchMore({
    variables: { offset: data.publicaciones.items.length, limit: 10 },
    updateQuery: (previousResult, { fetchMoreResult }) => {
      if (fetchMoreResult.publicaciones == null) {
        return previousResult;
      }

      return Object.assign({}, previousResult, {
        publicaciones: {
          __typename: previousResult.publicaciones.__typename,
          items: [...previousResult.publicaciones.items, ...fetchMoreResult.publicaciones.items],
          totalItems: fetchMoreResult.publicaciones.totalItems
        }
      });
    },
  });
...
export default AllPublicationsQuery; 