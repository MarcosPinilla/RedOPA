...
import { ALUMNOS_ELIMINADOS } from '../../../graphql/queries/estudianteseliminados';
import RestaurarAlumno from '../../../graphql/mutations/restaurarAlumno';
import VisibilityIcon from '@material-ui/icons/Visibility'

export default function EstudiantesEliminados(props) {
  ...
  const { loading, error, data, refetch } = useQuery(ALUMNOS_ELIMINADOS, 
    {
    ...
    <div className={classes.root}>
      ...
      { (error)
        ? <Grid container direction="column">
          <Typography variant="h5" style={{margin: 10}}>Estudiantes eliminados:</Typography>
        ...
        : <Grid container direction="column">
          <Typography variant="h5" style={{margin: 10}}>Estudiantes eliminados:</Typography>
          <Typography variant="h3" style={{margin: 10}} color="primary">{data.alumnosEliminadosQuery.length}</Typography>
          <Button variant="contained"
            color="primary"
            style={{margin: 10}}
          ...
          <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-titlex" >
            ...
            <DialogTitle id="form-dialog-titlex">Estudiantes eliminados</DialogTitle>
              <Grid item xs={12}>
              ...
              <Table className={classes.table}>
                ...
                <TableBody>
                {data.alumnosEliminadosQuery &&
                  data.alumnosEliminadosQuery.filter(filtro).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(alumno => (
                    <TableRow key={alumno.cuenta.rut}>
                      ...
                      <TableCell>
                        <RestaurarAlumno id={alumno.id} recargar={refetch} />
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              ...
              <TablePagination
                ...
                component="div"
                count={data.alumnosEliminadosQuery.length}
