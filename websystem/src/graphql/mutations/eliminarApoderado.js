...
export const ELIMINAR_APODERADO = gql`
  mutation deleteApoderado($apoderado_id: ID!) {
    deleteApoderado(apoderado_id: $apoderado_id)
  }
`;
...
const EliminarApoderado = (props) => {
  const classes = useStyles();
  const { id, eliminar, recargar } = props;
  const [open, setOpen] = React.useState(false);
  const [eliminarApoderado, {data, loading, error}] = useMutation(
    ELIMINAR_APODERADO,
    {
      onCompleted({ eliminarApoderado }) {
        setOpen(false);
        eliminar();
      }
    }
  ...
  return(
    <div>
      ...
      <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
        <DialogTitle id="form-dialog-title" style={{alignSelf: 'center'}}>Eliminar Apoderado</DialogTitle>
        ...
        <Button
          ...
          onClick={() => { eliminarApoderado({ variables: { apoderado_id: id } }) } }
        >
...
export default EliminarApoderado;